function GraphikChart(display, layout) {
    ...
    this.draw = function (data, config) {
        ...
        var drawChart = config.type === 'line' ? GraphikLineChart : GraphikBarChart

        var header = drawHeader(svg, config, layout.padding.left, layout.padding.top)
        var chart = drawChart(svg, config, layout, data, layout.padding.left, layout.padding.top + header.node().getBBox().height + layout.padding.prechart)
